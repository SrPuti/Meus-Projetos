...
public class MiniFullPvP extends Command{
				...
				for (int i = 0;i < connect.size();i++) {
					...
					if (rs.getDisplayName().equals(p.getDisplayName())) {
						if (connect.size() == 1) {
							...
							try {
								p.connect(BungeeCord.getInstance().getServerInfo("MiniFullPvP"));
								p.sendMessage(new TextComponent("§a Enviado para o servidor 'Full PVP' com sucesso."));
							} catch (Exception e) {
						...
						p.sendMessage(new TextComponent("§e Você está em §f#"+i+"§e lugar na fila do Full PVP."));
						}
					}
				}
			for (int i = 0; i< connect.size();i++) {
				Main.plugin.getProxy().getScheduler().schedule(Main.plugin, new Runnable() {
					...
					public void run() {
						...
						if (proximo != null) {
						try {
							proximo.connect(BungeeCord.getInstance().getServerInfo("MiniFullPvP"));
							p.sendMessage(new TextComponent("§a Enviado para o servidor 'Full PVP' com sucesso."));
						} catch (Exception e) {
							p.sendMessage(new TextComponent("§c Não foi possível conectar ao servidor."));
						}
